...
if user_choice == 'yes':
	...
	file.write(f"options = optimoptions('fmincon', 'Display', 'off','MaxFunctionEvaluations',{(n_edge*2+len(gm)+n_leaf)*2000},'MaxIterations',2000);\n")
	...
	file.write("F= -fval;\n")
	file.write(f"t = -log(x_optimal(1:{n_edge+len(gm)}));\n")
	file.write(f"c = exp(x_optimal({n_edge+n_leaf+len(gm)+1}:{n_edge*2+len(gm)+n_leaf}));\n")
	file.write(f"q = x_optimal({1+n_edge+len(gm)}:{n_edge+n_leaf+len(gm)});\n\nend\n\n")
